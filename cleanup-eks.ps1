...
Write-Host ""
Write-Host "WARNING: This will delete the ENTIRE EKS cluster and all resources created by CloudFormation!" -ForegroundColor Red
...
try {
    ...
    Write-Host "[OK] Stack deletion started successfully" -ForegroundColor Green
} catch {
...
Write-Host "This may take around 10–15 minutes..." -ForegroundColor Gray
Write-Host ""
...
while ($true) {
    ...
    if ($LASTEXITCODE -ne 0 -or $stackStatus -match "does not exist") {
        Write-Host ""
        Write-Host "[OK] Stack deleted successfully after $elapsed minutes!" -ForegroundColor Green
        break
    }
    ...
    if ($stackStatus -match "DELETE_FAILED") {
        ...
        aws cloudformation describe-stack-events `
            ...
            --output table
        Write-Host ""
        Write-Host "⚠️ Some resources (like EBS volumes or network interfaces) may need manual deletion." -ForegroundColor Yellow
        exit 1
    }
...
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "✅ All EKS resources from stack '$StackName' have been deleted successfully." -ForegroundColor White
Write-Host "Good job cleaning up, Amir! 🚀" -ForegroundColor Green
